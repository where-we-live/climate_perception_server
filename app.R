...
ui <- navbarPage(
  ...
  tabPanel("Models",
           sidebarLayout(
             sidebarPanel(
               selectInput("model_ycom_var", "YCOM Variable to Predict:", choices = available_variables),
               selectInput("model_clim_vars", "Climate Predictors:",
                           choices = c("aet", "def", "PDSI", "pet", "ppt", "q", "soil", "srad",
                                       "swe", "tmax", "tmin", "vap", "vpd", "ws"),
                           multiple = TRUE),
               actionButton("run_model", "Run Model"),
               textOutput("status_model")
             ),
             mainPanel(
...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_model, {
    ...
    output$status_model <- renderText("Running model...")
    
    tryCatch({
      # Load YCOM data
      ycom_df <- read.csv("data/YCOM_2024_publicdata.csv", fileEncoding = "latin1")
      ycom_df$FIPS <- str_pad(ycom_df$geoid, 5, pad = "0")
      
      # Load and summarize all selected climate predictors across years 2018-2024
      clim_dfs <- lapply(input$model_clim_vars, function(var) {
        df <- read.csv(paste0("data/", var, ".csv"), colClasses = c(FIPS = "character"))
        df$FIPS <- str_pad(df$FIPS, 5, pad = "0")
        df <- df %>% filter(year %in% 2018:2024) %>%
          group_by(FIPS) %>%
          summarize(mean_value = mean(.data[[var]], na.rm = TRUE)) %>%
          rename(!!var := mean_value)
      })
      
      clim_data <- Reduce(function(x, y) full_join(x, y, by = "FIPS"), clim_dfs)
      
      model_df <- ycom_df %>%
        filter(geotype == "county")
      
      if (!(input$model_ycom_var %in% colnames(model_df))) {
        stop(paste("Variable", input$model_ycom_var, "not found in YCOM data."))
      }
      
      model_df <- model_df %>%
        filter(!is.na(.data[[input$model_ycom_var]])) %>%
        select(FIPS, all_of(input$model_ycom_var)) %>%
        left_join(clim_data, by = "FIPS") %>%
        na.omit()
      
      formula_str <- paste(input$model_ycom_var, "~", paste(input$model_clim_vars, collapse = " + "))
      model <- lm(as.formula(formula_str), data = model_df)
      
      output$model_summary <- renderPrint(summary(model))
      
      output$model_plot <- renderPlot({
        plot(model$fitted.values, model_df[[input$model_ycom_var]],
             xlab = "Predicted", ylab = "Observed", main = "Observed vs Predicted",
             pch = 16, col = "darkblue")
        abline(0, 1, col = "red", lty = 2)
      })
      
      output$status_model <- renderText("Done.")
    }, error = function(e) {
      output$status_model <- renderText(paste("Error:", e$message))
    })
